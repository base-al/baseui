...
func createNewProject(cmd *cobra.Command, args []string) {
	...
	archiveURL := "https://github.com/base-al/baseui-core/archive/main.zip"
	...
	files, err := os.ReadDir(filepath.Join(projectName, "baseui-core-main"))
	if err != nil {
	...
	for _, f := range files {
		oldPath := filepath.Join(projectName, "baseui-core-main", f.Name())
	...
	os.RemoveAll(filepath.Join(projectName, "baseui-core-main"))
	...
	// enter new project directory and run `flutter pub get`
	...
	if output, err := flutterCmd.CombinedOutput(); err != nil {
		fmt.Printf("Error running 'flutter pub get': %v\n%s\n", err, output)
	}
